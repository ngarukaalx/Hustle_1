...
@app_views.route('/biz/<user_id>', methods=['GET'], strict_slashes=False)
def get_business_for_user(user_id):
    """Retrives a businesses for a user with all of its information
    -video
    -image
    -logo
    """
    all_business = [biz for biz in storage.all(Business).values() if biz.user_id == user_id]
    business_data = []
    for business in all_business:
        business_data_dict = business.to_dict()
        business_id = business_data_dict.get("id")
...
@app_views.route('/bizna/<business_id>', methods=['GET'], strict_slashes=False)
def by_id(business_id):
    """Retrives a businesses by its id
    -video
    -image
    -logo
    """
    print("amhere")
    all_business = [biz for biz in storage.all(Business).values() if biz.id == business_id]
    print(all_business)
    business_data = []
    for business in all_business:
        print('insidel oop: ', business)
        business_data_dict = business.to_dict()
        business_id = business_data_dict.get("id")
        county_id = business_data_dict.get("county_id")
        town_id = business_data_dict.get("town_id")
